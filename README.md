...
[![Packagist](https://img.shields.io/packagist/v/diviky/laravel-subscriptions.svg?label=Packagist&style=flat-square)](https://packagist.org/packages/diviky/laravel-subscriptions)
[![Scrutinizer Code Quality](https://img.shields.io/scrutinizer/g/diviky/laravel-subscriptions.svg?label=Scrutinizer&style=flat-square)](https://scrutinizer-ci.com/g/diviky/laravel-subscriptions/)
[![Travis](https://img.shields.io/travis/diviky/laravel-subscriptions.svg?label=TravisCI&style=flat-square)](https://travis-ci.org/diviky/laravel-subscriptions)
[![StyleCI](https://styleci.io/repos/93313402/shield)](https://styleci.io/repos/93313402)
[![License](https://img.shields.io/packagist/l/diviky/laravel-subscriptions.svg?label=License&style=flat-square)](https://github.com/diviky/laravel-subscriptions/blob/develop/LICENSE)
...
1. Install the package via composer:
    ```shell
    composer require diviky/laravel-subscriptions
    ```

2. Publish resources (migrations and config files):
    ```shell
    php artisan diviky:publish:subscriptions
    ```

3. Execute migrations via the following command:
    ```shell
    php artisan diviky:migrate:subscriptions
    ```
...
**Diviky Subscriptions** has been specially made for Eloquent and simplicity has been taken very serious as in any other Laravel related aspect. To add Subscription functionality to your User model just use the `\Diviky\Subscriptions\Concerns\HasPlanSubscriptions` trait like this:

```php
namespace App\Models;

use Diviky\Subscriptions\Concerns\HasPlanSubscriptions;
...
```php
$plan = app('diviky.subscriptions.plan')->create([
    'name' => 'Pro',
...
```php
$plan = app('diviky.subscriptions.plan')->find(1);
...
$amountOfPictures = app('diviky.subscriptions.plan_feature')->where('slug', 'pictures_per_listing')->first()->value;

// Get feature value through the subscription instance
$amountOfPictures = app('diviky.subscriptions.plan_subscription')->find(1)->getFeatureValue('pictures_per_listing');
```
...
```php
$plan = app('diviky.subscriptions.plan')->find(2);
$subscription = app('diviky.subscriptions.plan_subscription')->find(1);
...
```php
// Find plan feature
$feature = app('diviky.subscriptions.plan_feature')->where('name', 'listing_duration_days')->first();
...
$subscriptions = app('diviky.subscriptions.plan_subscription')->byPlanId($plan_id)->get();

// Get bookings of the given user
$user = \App\Models\User::find(1);
$bookingsOfSubscriber = app('diviky.subscriptions.plan_subscription')->ofSubscriber($user)->get(); 

// Get subscriptions with trial ending in 3 days
$subscriptions = app('diviky.subscriptions.plan_subscription')->findEndingTrial(3)->get();

// Get subscriptions with ended trial
$subscriptions = app('diviky.subscriptions.plan_subscription')->findEndedTrial()->get();

// Get subscriptions with period ending in 3 days
$subscriptions = app('diviky.subscriptions.plan_subscription')->findEndingPeriod(3)->get();

// Get subscriptions with ended period
$subscriptions = app('diviky.subscriptions.plan_subscription')->findEndedPeriod()->get();
```

### Models

**Diviky Subscriptions** uses 4 models:

```php
Diviky\Subscriptions\Models\Plan;
Diviky\Subscriptions\Models\PlanFeature;
Diviky\Subscriptions\Models\PlanSubscription;
Diviky\Subscriptions\Models\PlanSubscriptionUsage;
```
...
- [Chat on Slack](https://bit.ly/diviky-slack)
- [Help on Email](mailto:help@diviky.com)
- [Follow on Twitter](https://twitter.com/diviky)
...
If you discover a security vulnerability within this project, please send an e-mail to [help@diviky.com](help@diviky.com). All security vulnerabilities will be promptly addressed.


## About Diviky

Diviky is a software solutions startup, specialized in integrated enterprise solutions for SMEs established in Alexandria, Egypt since June 2016. We believe that our drive The Value, The Reach, and The Impact is what differentiates us and unleash the endless possibilities of our philosophy through the power of software. We like to call it Innovation At The Speed Of Life. That’s how we do our share of advancing humanity.
...
(c) 2016-2022 Diviky LLC, Some rights reserved.
